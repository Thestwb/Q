...
# 使用训练的模型进行预测，为了方便，
X_test=[(13.52,3.17,2.72,23.5,97),( 12.42,2.55,2.27,22,90),( 13.76,1.53,2.7,19.5,132)]# 案例直接把测试集里面的数据拿出来三条
print(X_test[0:3])#利用3个数据进行测试，即取3个数据作为模型的输入端
model.predict(X_test[0:3])
CN={"Zinfandel":"仙粉黛","Syrah":"西拉","Sauvignon":"赤霞珠"}
a=model.predict(X_test[0:3])
for i in range(3):
	a[i]=CN[a[i]]
print(a)#输出测试的结果，即输出模型预测的结果
